        ...
        <b-navbar-nav class="ml-auto t-sha">
          ...
          <b-nav-item-dropdown>
            <template slot="button-content">
              <b-img :src="this.$root.credentials[1]" class="user"/>
              <a class="decor-a">{{this.$root.credentials[0]}}</a>
            </template>
            <b-dropdown-item
              class="backg rout2"
              v-if="!this.$session.exists()"
              @click="this.existSession"
            >Log in</b-dropdown-item>
            <b-dropdown-item class="backg" v-if="this.$session.exists()">
              <router-link to="/profile" class="rout2">
                <b-img :src="this.$root.credentials[1]" class="useritem"/>
                <a class="decor-a">Profile</a>
              </router-link>
            </b-dropdown-item>
            <b-dropdown-item
              class="backg rout2"
          ...
          <b-nav-form>
            ...
            />
            <b-button
              @click="this.getArcticlebyName"
              size="sm"
              class="my-2 my-sm-0 bg-info bt round"
              type="submit"
            >Search</b-button>
          </b-nav-form>
        </b-navbar-nav>
    ...
    >
      <b-container class="container">
        <b-form v-if="show">
          ...
          <router-link to="/pages/register" class="rout">
            <b-button block variant="info" @click="this.closeModal" class="marginbott">Register</b-button>
          </router-link>
        </b-form>
      </b-container>
    </b-modal>
...
export default {
  ...
  created() {
    if (this.$session.exists())
      this.$root.credentials = [
        this.$session.get("credent")[1],
        this.$session.get("credent")[7]
      ];
  ...
  methods: {
    getArcticlebyName() {
      this.$toastr.info(
        'Acabas de buscar articulos relacionados con "' +
          this.nameArticle +
          '"',
        "Search"
      );
    ...
    logIn() {
      if (this.form.email != "" && this.form.pass != "") {
        fetch("/api/person/" + this.form.email + "&" + this.form.pass)
          ...
          .then(data => {
            if (data[0]) {
              ...
              this.$forceUpdate();
              this.$toastr.info(
                "Bienvenido " + this.$root.credentials[0],
                "Log in"
              );
              this.closeModal();
            } else {
        ...
        this.$toastr.warning("Todos los campos son obligatorios", "Log in");
    },
    logOut() {
      this.$toastr.info(
        "Vuelve pronto " + this.$root.credentials[0],
        "Te extrañaremos"
      );
